...
int load_mode(char *data, struct mipi_dsi_device *dsi, struct generic_panel *ctx)
{
	...
	if ((mode->clock > 5000) && (mode->vertical[1] > 0) && (mode->horizontal[1] > 0)) {
		...
		dev_dbg(dev, "Mode %d %d %d  %p -> %p\n", mode->clock,
			mode->vertical[1], mode->horizontal[1], mode, mode->prev);
		return 0;
	}
	return -1;
}
...
{
	int ret;
	struct generic_panel_init_seq *rev, *fwd, *tmp;

	ret = panel_description_foreach(dsi, ctx, &load_panel_description_line);
	if (ret < 0)
		return ret;

	/* Reverse iseq (was built as a stack) */
	rev = ctx->iseq;
	fwd = NULL;
	tmp = NULL;
	while (rev) {
...
static int generic_panel_init_sequence(struct generic_panel *ctx)
{
	...
	while (iseq) {
		if (iseq->read > 0) {
			...
			if (iseq->read > 8)
				iseq->read = 8;
			ret = mipi_dsi_generic_read(dsi, iseq->data, iseq->len,
						    &readbuf[0], iseq->read);
			if (ret < 0) {
				   ...
				   struct drm_connector *connector)
{
	...
	while (genmode) {
		dev_dbg(ctx->dev, "gen mode %d %dx%d\n", genmode->clock,
			genmode->horizontal[1], genmode->vertical[1]);
